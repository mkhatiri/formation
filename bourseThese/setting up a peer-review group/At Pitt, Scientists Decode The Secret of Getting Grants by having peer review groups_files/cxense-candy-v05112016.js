var cxCandyObj = {
    ...
    reportEvent: function (explicitUrl, extraData, explicitSiteId) {
        var locData = null;
        ...
        cX.setSiteId(explicitSiteId || siteId);
    ...
    showCandyBar: function() {
        var divName = (cxdj.meta.isHP === 'y') ?  'cx-candybarhp' : 'cx-candybar';
        var renderTemplate = cxdj.isSubscriber(cxdj.meta.subscriber) ?  'wsj-candybar-subscriber.html' : 'wsj-candybar' + cxdj.envType + '.html';
        var div = cxdj.getDisplayDiv('#' + divName);

        if (div) {
            div.css({position: 'fixed', bottom:0, width: 0, height: 0, 'z-index' : 8000, display: 'none'});
            cX.callQueue.push(['insertWidget', {
                appendToElementId: divName,
                width: 0, height: 0,
    ...
    showPopTart: function() {
        ...
        if (div) {
            ...
            cX.callQueue.push(['insertWidget', {
                ...
                width: 0, height: 0,
                renderTemplateUrl: cxdj.renderTemplateHost + 'wsj-poptart' + cxdj.envType + '.html',
                meta: cX.JSON.stringify(cxdj.meta)
            }]);
    ...
    showScrim: function () {
        ...
        if (div) {
            var renderTemplate = cX.getTopLevelDomain() == 'wsjplus.com'  ? 'wsjplus-scrim'  + cxdj.envType + '.html' :'wsj-scrim'  + cxdj.envType + '.html';

            div.css({position:'fixed', top:0, left:0, width: 0, height: 0, display: 'none', 'z-index' : 8000});
            cX.callQueue.push(['insertWidget', {
    ...
    showSnippet: function () {
        ...
        if (div) {
            ...
            cX.callQueue.push(['insertWidget', {
                ...
                renderTemplateUrl: cxdj.renderTemplateHost + 'wsj-snippet' + cxdj.envType + '.html',
                meta: cX.JSON.stringify(cxdj.meta)
            }]);
        }
    },
    showInterstitial: function () {
        ...
        if (div) {
            ...
            cX.callQueue.push(['insertWidget', {
                ...
                renderTemplateUrl: cxdj.renderTemplateHost + 'barrons-interstitial-snippet' + cxdj.envType + '.html',
                meta: cX.JSON.stringify(cxdj.meta)
            }]);
        }
    },
    ...
    scrimPresentation: function(divId, showThreshold) {

        var showingScrim = false;
        $(divId).hide();
        $(divId + ' iframe').hide();
        ...
        $(window).bind("scroll dynamicScroll", function(){
            ...
            if( current >= showPosition  && !showingScrim){
                $(divId).stop(true).height('100%').width('100%');
                $(divId + ' iframe').stop(true).width('100%').height('100%').show();
                $(divId).fadeIn(500);
                showingScrim = true;
            }
        }).trigger("dynamicScroll");
    ...
    renderCallback: function(event) {
        if (event && event.data && typeof event.data === 'string' && cX && cX.decodeUrlEncodedNameValuePairs) {
            ...
            if (eventData.type === 'hp') {
                ...
                candyId = '#cx-scrim';
            } else if (eventData.type == 'poptart') {
                candyId = '#cx-poptart'
            }
            ...
            switch (eventData.method) {
                ...
                case 'cxCandyShow':
                    ...
                    if ((eventData.contentHeight === '100%')  || $.isNumeric(eventData.contentHeight)) {
                        $(candyId).height(eventData.contentHeight);
                    }
                    if ((eventData.contentWidth === '100%') || $.isNumeric(eventData.contentWidth)) {
                        $(candyId).width(eventData.contentWidth);
                    }
                    break;
                case 'cxCandyHide':
                    $(candyId).height(0).width(0).hide();
                    $(candyId + ' iframe').height(0).width(0).hide();
                    break;

                case 'cxCandyShowOnScroll':
                    cxdj.scrimPresentation(candyId, 0.5);
                    break;
            }
        }
    },
...
var cxad = {
    ...
    buildKTarget : function(meta) {
        var kTarget = 'wsj' + meta.subscriber;

        if (meta.ctp) {
            kTarget += ' wsjctp' + meta.ctp.toLowerCase();
        }

        if (meta.navSource) {
            kTarget += ' wsjmod' + meta.navSource;
        }

        if (typeof meta.bucket !== 'undefined') {
            kTarget += ' wsjgroup' + meta.bucket;
        }

        if (meta.userExp) {
            kTarget += ' wsjux' + meta.userExp.replace(/_| /g, '');
        }
        ...
        if (meta.referrer) {
            kTarget += ' wsjref' + meta.referrer;
        }

        return kTarget;
    },

    getHost : function(meta) {
        var referrer = document.referrer || '';
        var host = referrer.replace(/^https?:\/\//, '').split('/')[0];
